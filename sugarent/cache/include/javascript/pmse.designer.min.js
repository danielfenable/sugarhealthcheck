...
var AdamShape=function(options){jCore.CustomShape.call(this,options);this.label=this.labels.get(0);this.markersArray=new jCore.ArrayList();};AdamShape.prototype=new jCore.CustomShape();AdamShape.prototype.type="AdamShape";AdamShape.prototype.adamConnectionDropBehavior=null;AdamShape.prototype.getType=function(){return this.type;};AdamShape.prototype.setName=function(value){var item;if(this.label){this.label.setMessage(value);if(listPanelError){if(listPanelError.items.length){item=listPanelError.getItemById(this.id);if(item){if(value.trim().length){item.setTitle(value);}else{item.setTitle("[unnamed]");}}}}}
return this;};AdamShape.prototype.saveProject=function(root,App,w){root.canvas.showModal();App.alert.show('upload',{level:'process',title:'LBL_LOADING',autoClose:false});root.canvas.project.save({success:function(){root.canvas.hideModal();w.show();w.html.style.display='none';}});};AdamShape.prototype.getName=function(){var text="";if(this.label){text=this.label.getMessage();}
...
return this.connectionContainerDropBehavior;}};AdamShape.prototype.getDestElements=function(){var elements=[],i,port,connection;for(i=0;i<this.getPorts().getSize();i+=1){port=this.getPorts().get(i);connection=port.connection;if(connection.srcPort.parent.getID()===this.getID()){elements.push(connection.destPort.parent);}}
return elements;};AdamShape.prototype.setDefaultFlow=function(floID){var i,port,connection;for(i=0;i<this.getPorts().getSize();i+=1){port=this.getPorts().get(i);connection=port.connection;this.updateDefaultFlow(0);if(connection.srcPort.parent.getID()===this.getID()){if(connection.getID()===floID){this.updateDefaultFlow(floID);connection.setFlowCondition("");connection.changeFlowType('default');connection.setFlowType("DEFAULT");}else if(connection.getFlowType()==='DEFAULT'){connection.changeFlowType('sequence');connection.setFlowType("SEQUENCE");}}}
return this;};AdamShape.prototype.updateFlowConditions=function(){var i,connection,updatedElement;for(i=0;i<this.getPorts().getSize();i+=1){connection=this.getPorts().get(i).connection;if(connection.flo_element_origin===this.getID()){if(connection.flo_condition&&connection.flo_condition!==''){updatedElement=[{id:connection.getID(),type:connection.type,relatedObject:connection,fields:[{field:"condition",newVal:'',oldVal:connection.getFlowCondition()}]}];connection.setFlowCondition('');connection.getCanvas().triggerDefaultFlowChangeEvent(updatedElement);}}}};AdamShape.prototype.getFamilyNumber=function(shape){var map={'AdamActivity':5,'AdamEvent':6,'AdamGateway':7,'AdamData':8,'AdamArtifact':9};return map[shape.getType()];};AdamShape.prototype.attachErrorToShape=function(objArray){var i,j,sw,message,ruleArray,testCount,rule,error,sizeItems,allErrors=[];this.BPMNError=new jCore.ArrayList();for(i=0;i<objArray.length;i+=1){message=objArray[i].message;ruleArray=objArray[i].rules;sw=(objArray[i].family===8&&objArray[i].familyType===4)?false:true;for(j=0;j<ruleArray.length;j+=1){rule=ruleArray[j];testCount=this.countFlow(rule.element,rule.direction);if(objArray[i].family===8&&objArray[i].familyType===4){sw=sw||(testCount>rule.value);}else if(objArray[i].family===5&&objArray[i].familyType===1&&objArray[i].familySubType===1){switch(rule.compare){case'!=':sw=sw&&('NONE'!==rule.value);break;}}else{switch(rule.compare){case'=':sw=sw&&(testCount===rule.value);break;case'>':sw=sw&&(testCount>rule.value);break;case'<':sw=sw&&(testCount<rule.value);break;}}}
if(!sw){this.BPMNError.insert({code:objArray[i].id,direction:rule.direction,description:message});}}
if(this.BPMNError.getSize()>0){this.addErrorLayer('error',2);}else{this.clearErrors();}
listPanelError.setItems(this.getShapeWithErros());if(countErrors){if(listPanelError.getItems().length){countErrors.style.display="block";sizeItems=listPanelError.getAllErros();countErrors.textContent=sizeItems===1?sizeItems+translate('LBL_PMSE_BPMN_WARNING_SINGULAR_LABEL'):sizeItems+translate('LBL_PMSE_BPMN_WARNING_LABEL');}else{countErrors.textContent="0"+translate('LBL_PMSE_BPMN_WARNING_SINGULAR_LABEL');}}};AdamShape.prototype.getShapeWithErros=function(){var i,shape,errors,error,f=[],items=[],object={},subObject={};for(i=0;i<canvas.getCustomShapes().getSize();i+=1){shape=canvas.getCustomShapes().get(i);if(shape.BPMNError!==undefined){if(shape.BPMNError.getSize()){object={};object.title=shape.getName()||'[unnamed]';object.errorType=this.getShapeType(shape.getType(),shape);object.items=[];object.errorId=shape.getID();errors=shape.BPMNError;for(j=0;j<errors.getSize();j+=1){subObject={};error=errors.get(j);subObject.messageId=error.code;subObject.message=error.description;object.items.push(subObject);}
items.push(object);}}}
return items;};AdamShape.prototype.getShapeType=function(type,shape){var shapeType,shapeMessage,itemType="";switch(type){case"AdamActivity":shapeType=shape.act_task_type;itemType=type+shapeType;break;case"AdamEvent":shapeType=shape.getEventType();shapeMessage=shape.getEventMessage()||shape.getEventMarker();itemType=type+shapeType+shapeMessage;break;case"AdamGateway":shapeType=shape.getGatewayType();shapeMessage=shape.getGatewayType();itemType=type+shapeType;break;};return itemType;};AdamShape.prototype.countFlow=function(element,direction){var i,eleMap={'sequenceFlow':'regular','associationFlow':'dotted'},port,connection,count=0;for(i=0;i<this.getPorts().getSize();i+=1){port=this.getPorts().get(i);connection=port.getConnection();switch(direction){case'incoming':if(eleMap[element]===connection.segmentStyle){if(port.getID()===connection.getDestPort().getID()){count+=1;}}
...
break;}}
return count;};AdamShape.prototype.addErrorLayer=function(cssMarker,position){var layer,cl,cs,zoom,options;layer=this.layers.find('id',this.id+'Layer-Errors');if(typeof position==='undefined'||position===null){cl=cssMarker;cs='bpmn_zoom';}else{cl='div-empty';cs='';}
...
if(typeof position!=='undefined'&&position!==null){this.addErrors(layer,position);}};AdamShape.prototype.clearErrors=function(){var i,lMarker,ifExist;for(i=0;i<this.markersArray.getSize();i+=1){lMarker=this.markersArray.get(i);if(lMarker.position===2){lMarker.removeAllClasses();break;}}};AdamShape.prototype.addErrors=function(newLayer,pos){var nMarker,x,lMarker,ifExist=false,errorArrayClass=[],cls,i;for(i=0;i<this.markersArray.getSize();i+=1){lMarker=this.markersArray.get(i);if(lMarker.position===pos){ifExist=true;break;}}
for(i=0;i<newLayer.ZOOMSCALES;i+=1){cls='adam-status-'+((i*25)+50)+'-warning adam-error-color fa fa-exclamation-circle';errorArrayClass.push(cls);}
if(!ifExist){nMarker=new AdamMarker({parent:newLayer,position:pos,height:17,width:17,markerZoomClasses:errorArrayClass});this.markersArray.insert(nMarker);nMarker.paint();nMarker.setElementClass(errorArrayClass);}else{lMarker.setElementClass(errorArrayClass);}};
...
this.loaded=true;}else{this.loaded=false;}};AdamProject.prototype.addElement=function(element){var object,pk_name,list,i,pasteElement;if(element.relatedElements.length>0){for(i=0;i<element.relatedElements.length;i+=1){pasteElement=element.relatedElements[i];list=this.getUpdateList(pasteElement.type);object=pasteElement.getDBObject();object.action="CREATE";list[pasteElement.id]=object;}}else{pk_name=this.formatProperty(element.type,'uid');list=this.getUpdateList(element.type);element.relatedObject[pk_name]=element.id;object=element.relatedObject.getDBObject();object.action="CREATE";list[element.id]=object;}
...
return cleaned;};AdamCanvas.prototype.bpmnValidation=function(){var i,j,shape,rulesObject=this.bpmnRules,family,rules,message,testCount,objArray,sw;for(i=0;i<this.getCustomShapes().getSize();i+=1){objArray=[];shape=this.getCustomShapes().get(i);family=shape.getFamilyNumber(shape);switch(family){case 5:if((shape.getActivityType()==='TASK'||shape.getActivityType()==='SUBPROCESS')&&shape.getActivityTaskType()!='SCRIPTTASK'){objArray=rulesObject[shape.getType()]
['task'];}else if(shape.getActivityTaskType()==='SCRIPTTASK'){objArray=rulesObject[shape.getType()]
['scripttask'];objArray[2].rules[0].value=shape.getActivityScriptType();}
break;case 6:if(rulesObject[shape.getType()]&&rulesObject[shape.getType()]
[shape.getEventType().toLowerCase()]){objArray=rulesObject[shape.getType()]
[shape.getEventType().toLowerCase()];}
break;case 7:switch(shape.getDirection()){case'CONVERGING':objArray=rulesObject[shape.getType()]
['converging'];break;case'DIVERGING':objArray=rulesObject[shape.getType()]
['diverging'];break;case'MIXED':objArray=rulesObject[shape.getType()]
['mixed'];break;}
break;case 9:if(rulesObject[shape.getType()]&&rulesObject[shape.getType()]
[shape.getArtifactType().toLowerCase()]){objArray=rulesObject[shape.getType()]
...
convert={label:translate('LBL_PMSE_CONTEXT_MENU_CONVERT'),icon:'adam-menu-icon-convert',menu:{items:items}};items=[];directionActive=(this.gat_direction);noneDirectionAction=new Action({text:translate('LBL_PMSE_CONTEXT_MENU_UNSPECIFIED'),cssStyle:'adam-menu-icon-none',handler:function(){self.updateDirection('UNSPECIFIED');},disabled:unspecifiedDirectionActive});convergingDirectionAction=new Action({text:translate('LBL_PMSE_CONTEXT_MENU_CONVERGING'),cssStyle:'adam-menu-icon-gateway-converging',handler:function(){self.updateDirection('CONVERGING');self.cleanFlowConditions();},disabled:convergingDirectionActive});divergingDirectionAction=new Action({text:translate('LBL_PMSE_CONTEXT_MENU_DIVERGING'),cssStyle:'adam-menu-icon-gateway-diverging',handler:function(){self.updateDirection('DIVERGING');},disabled:divergingDirectionActive});mixedDirectionAction=new Action({text:translate('LBL_PMSE_CONTEXT_MENU_MIXED'),cssStyle:'adam-menu-icon-gateway-mixed',handler:function(){self.updateDirection('MIXED');},disabled:mixedDirectionActive});direction={label:translate('LBL_PMSE_CONTEXT_MENU_DIRECTION'),icon:'adam-menu-icon-gateway-direction',menu:{items:[convergingDirectionAction,divergingDirectionAction]}};if(elements.length>1){if(configurateAction.size>0){items.push(configurateAction.action,{jtype:'separator'});}
...
return this;};AdamActivity.prototype.paint=function(){var m,marker;AdamShape.prototype.paint.call(this);for(m=0;m<this.markersArray.getSize();m+=1){marker=this.markersArray.get(m);marker.paint();}};AdamActivity.prototype.getActivityType=function(){return this.act_type;};AdamActivity.prototype.getContextMenu=function(){var self=this,deleteAction,usertaskAction,scriptAction,configureAction,assignUsersAction,elements=this.getDestElements(),defaultflowActive=(elements.length>1)?false:true,defaultflownoneAction,defaultflowItems=[],name,items,i,shape,handle,port,connection,actionItems=[],noneAction,assignUserAction,assignTeamAction,changeFieldAction,addRelatedRecordAction,businessRuleAction,defaultflowAction;deleteAction=new Action({text:translate('LBL_PMSE_CONTEXT_MENU_DELETE'),cssStyle:'adam-menu-icon-delete',handler:function(){var shape;shape=self.canvas.customShapes.find('id',self.id);if(shape){shape.canvas.emptyCurrentSelection();shape.canvas.addToSelection(shape);shape.canvas.removeElements();}}});noneAction=new Action({text:translate('LBL_PMSE_CONTEXT_MENU_UNASSIGNED'),cssStyle:'adam-menu-script-none',handler:function(){self.updateScriptType('NONE');},disabled:(this.act_script_type==='NONE')});assignUserAction=new Action({text:translate('LBL_PMSE_CONTEXT_MENU_ASSIGN_USER'),cssStyle:'adam-menu-script-assign_user',handler:function(){self.updateScriptType('ASSIGN_USER');},disabled:(this.act_script_type==='ASSIGN_USER')});assignTeamAction=new Action({text:translate('LBL_PMSE_CONTEXT_MENU_ASSIGN_TEAM'),cssStyle:'adam-menu-script-assign_team',handler:function(){self.updateScriptType('ASSIGN_TEAM');},disabled:(this.act_script_type==='ASSIGN_TEAM')});changeFieldAction=new Action({text:translate('LBL_PMSE_CONTEXT_MENU_CHANGE_FIELD'),cssStyle:'adam-menu-script-change_field',handler:function(){self.updateScriptType('CHANGE_FIELD');},disabled:(this.act_script_type==='CHANGE_FIELD')});addRelatedRecordAction=new Action({text:translate('LBL_PMSE_CONTEXT_MENU_ADD_RELATED_RECORD'),cssStyle:'adam-menu-script-add_related_record',handler:function(){self.updateScriptType('ADD_RELATED_RECORD');},disabled:(this.act_script_type==='ADD_RELATED_RECORD')});businessRuleAction=new Action({text:translate('LBL_PMSE_CONTEXT_MENU_BUSINESS_RULE'),cssStyle:'adam-menu-script-business_rule',handler:function(){self.updateScriptType('BUSINESS_RULE');},disabled:(this.act_script_type==='BUSINESS_RULE')});if(this.act_task_type==='USERTASK'){configureAction=this.createConfigurateAction();assignUsersAction=this.createAssignUsersAction();}else{configureAction=this.actionFactory(this.act_script_type);}
if(elements.length>1){handle=function(id){return function(){var cmd=new CommandDefaultFlow(self,id);cmd.execute();self.canvas.commandStack.add(cmd);};};defaultflownoneAction=new Action({text:translate('LBL_PMSE_CONTEXT_MENU_NONE'),cssStyle:'adam-menu-icon-none',handler:handle(""),disabled:(self.act_default_flow!==0)?false:true});defaultflowItems.push(defaultflownoneAction);for(i=0;i<this.getPorts().getSize();i+=1){port=this.getPorts().get(i);connection=port.connection;if(connection.srcPort.parent.getID()===this.getID()){shape=connection.destPort.parent;switch(shape.getType()){case'AdamActivity':name=(shape.getName()!=='')?shape.getName():translate('LBL_PMSE_CONTEXT_MENU_DEFAULT_TASK');break;case'AdamEvent':name=(shape.getName()!=='')?shape.getName():translate('LBL_PMSE_CONTEXT_MENU_DEFAULT_EVENT');break;case'AdamGateway':name=(shape.getName()!=='')?shape.getName():translate('LBL_PMSE_CONTEXT_MENU_DEFAULT_GATEWAY');break;}
defaultflowItems.push(new Action({text:name,cssStyle:self.getCanvas().getTreeItem(shape).icon,handler:handle(connection.getID()),disabled:(self.act_default_flow===connection.getID())?true:false}));}}
defaultflowAction={label:translate('LBL_PMSE_CONTEXT_MENU_DEFAULT_FLOW'),icon:'adam-menu-icon-default-flow',disabled:defaultflowActive,menu:{items:defaultflowItems}};}
items=[configureAction];if(this.act_task_type==='USERTASK'){items.push({jtype:'separator'},assignUsersAction);}
items.push({jtype:'separator'});if(this.act_task_type==='SCRIPTTASK'){actionItems.push(noneAction);actionItems.push(businessRuleAction);actionItems.push(assignUserAction,assignTeamAction,changeFieldAction,addRelatedRecordAction);items.push({label:translate('LBL_PMSE_CONTEXT_MENU_ACTION_TYPE'),icon:'adam-menu-icon-convert',menu:{items:actionItems}},{jtype:'separator'});}
...
f=new Form({items:items,closeContainerOnSubmit:true,buttons:[{jtype:'submit',caption:translate('LBL_PMSE_BUTTON_SAVE'),cssClasses:['btn btn-primary']},{jtype:'normal',caption:translate('LBL_PMSE_BUTTON_CANCEL'),handler:function(){if(f.isDirty()){cancelInformation.setButtons([{jtype:'normal',caption:translate('LBL_PMSE_BUTTON_YES'),handler:function(){cancelInformation.hide();w.close();}},{jtype:'normal',caption:translate('LBL_PMSE_BUTTON_NO'),handler:function(){cancelInformation.hide();}}]);cancelInformation.show();}else{w.close();}},cssClasses:['btn btn-invisible btn-link']}],callback:callback,language:PMSE_DESIGNER_FORM_TRANSLATIONS});w.addPanel({title:translate('LBL_PMSE_FORM_LABEL_GENERAL_SETTINGS'),panel:f});if(f2){w.addPanel({title:translate('LBL_PMSE_FORM_LABEL_READ_ONLY_FIELDS'),panel:f2});}
...
action=new Action({text:actionName,cssStyle:actionCSS,handler:function(){root.saveProject(root,App,w);},disabled:disabled});return action;};AdamActivity.prototype.updateScript=function(script){var updateCommand;updateCommand=new CommandAdam(this,['act_script'],[script]);updateCommand.execute();this.canvas.commandStack.add(updateCommand);return this;};AdamActivity.prototype.createAssignUsersAction=function(){var action,root=this,w,f,items,assignUserField,assignTeamField,combo_users,combo_teams,combo_method,hiddenFn,hiddenUpdateFn,callback,proxy,teams,users,cancelInformation,self=this;cancelInformation=new MessagePanel({title:"Confirm",wtype:'Confirm',message:translate('LBL_PMSE_MESSAGE_CANCEL_CONFIRM')});proxy=new SugarProxy({url:'pmse_Project/ActivityDefinition/'+this.id,uid:this.id,callback:null});hiddenFn=function(){if(combo_method.value==='static'){combo_users.enable();combo_teams.setReadOnly(true);}else{combo_users.disable().setValue("");combo_teams.setReadOnly(false);}};hiddenUpdateFn=function(){switch(this.name){case'combo_teams':assignTeamField.setValue(combo_teams.value);assignUserField.setValue(null);break;case'combo_users':assignTeamField.setValue(null);assignUserField.setValue(combo_users.value);break;}};assignTeamField=new HiddenField({name:'act_assign_team'});assignUserField=new HiddenField({name:'act_assign_user'});combo_users=new SearchableCombobox({label:translate('LBL_PA_FORM_LABEL_ASSIGN_TO_USER'),name:'combo_users',submit:false,change:hiddenUpdateFn,disabled:true,searchURL:'pmse_Project/CrmData/users?filter={TERM}',options:[{'text':translate('LBL_PMSE_FORM_OPTION_CURRENT_USER'),'value':'currentuser'},{'text':translate('LBL_PMSE_FORM_OPTION_RECORD_OWNER'),'value':'owner'},{'text':translate('LBL_PMSE_FORM_OPTION_SUPERVISOR'),'value':'supervisor'}]});combo_teams=new ComboboxField({jtype:'combobox',label:translate('LBL_PA_FORM_LABEL_ASSIGN_TO_TEAM'),name:'combo_teams',submit:false,change:hiddenUpdateFn,proxy:new SugarProxy({url:'pmse_Project/CrmData/teams/public',uid:'public',callback:null})});combo_method=new ComboboxField({jtype:'combobox',name:'act_assignment_method',label:translate('LBL_PMSE_FORM_LABEL_ASSIGNMENT_METHOD'),change:hiddenFn,options:[{text:translate('LBL_PMSE_FORM_OPTION_ROUND_ROBIN'),value:'balanced'},{text:translate('LBL_PMSE_FORM_OPTION_SELF_SERVICE'),value:'selfservice'},{text:translate('LBL_PMSE_FORM_OPTION_STATIC_ASSIGNMENT'),value:'static'}],initialValue:'balanced',required:true});callback={'submit':function(data){fData=f.getData();proxy.sendData(fData);},'loaded':function(data){proxy.getData({'module':PROJECT_MODULE},{success:function(data){var aUsers=[{'text':translate('LBL_PMSE_FORM_OPTION_CURRENT_USER'),'value':'currentuser'},{'text':translate('LBL_PMSE_FORM_OPTION_RECORD_OWNER'),'value':'owner'},{'text':translate('LBL_PMSE_FORM_OPTION_SUPERVISOR'),'value':'supervisor'}],usersProxy=new SugarProxy();root.canvas.emptyCurrentSelection();combo_teams.proxy.getData(null,{success:function(teams){combo_teams.setOptions(teams.result);assignTeamField.setValue(data.act_assign_team||teams.result[0].value);}});usersProxy.url='pmse_Project/CrmData/users';usersProxy.getData(null,{success:function(users){var i,theMatch;users=users.result||[];for(i=0;i<users.length;i++){if(users[i].value===data.act_assign_user){theMatch={text:users[i].text,value:users[i].value};}}
if(!theMatch){theMatch={text:data.act_assign_user,value:data.act_assign_user};}
combo_users.setValue(theMatch);App.alert.dismiss('upload');w.html.style.display='inline';}});assignUserField.setValue(data.act_assign_user);if(data){combo_method.setValue(data.act_assignment_method);if(data.act_assignment_method==='static'){combo_users.setValue(data.act_assign_user);combo_users.enable();combo_teams.setReadOnly(true);}else{combo_teams.setValue(data.act_assign_team);combo_users.disable();combo_teams.setReadOnly(false);}}
f.proxy=null;}});}};f=new Form({items:[combo_method,combo_teams,combo_users,assignUserField,assignTeamField],closeContainerOnSubmit:true,buttons:[{jtype:'submit',caption:translate('LBL_PMSE_BUTTON_SAVE'),cssClasses:['btn btn-primary']},{jtype:'normal',caption:translate('LBL_PMSE_BUTTON_CANCEL'),handler:function(){if(f.isDirty()){cancelInformation.setButtons([{jtype:'normal',caption:translate('LBL_PMSE_BUTTON_YES'),handler:function(){cancelInformation.hide();w.close();}},{jtype:'normal',caption:translate('LBL_PMSE_BUTTON_NO'),handler:function(){cancelInformation.hide();}}]);cancelInformation.show();}else{w.close();}},cssClasses:['btn btn-invisible btn-link']}],callback:callback,language:PMSE_DESIGNER_FORM_TRANSLATIONS});w=new Window({width:500,height:350,title:translate('LBL_PMSE_FORM_TITLE_USER_DEFINITION')+': '+this.getName(),modal:true});w.addPanel(f);action=new Action({text:translate('LBL_PMSE_CONTEXT_MENU_USERS'),cssStyle:'adam-menu-icon-user',handler:function(){root.saveProject(root,App,w);},disabled:false});return action;};AdamActivity.prototype.actionFactory=function(type){var action,actionText,actionCSS,root=this,w,f,proxy=null,items=[],callback={},disabled=false,windowTitle='',wWidth,wHeight,labelWidth='30%',messageMap,fields,combo_users,combo_teams,combo_method,navigableData,changeFieldsFn,combo_modules,data,restored_fields,fields_updater,initialModule,combo_business,i,hidden_module,cancelInformation,hidden_method,updater_field,updateRecordOwner;cancelInformation=new MessagePanel({title:"Confirm",wtype:'Confirm',message:translate('LBL_PMSE_MESSAGE_CANCEL_CONFIRM')});switch(type){case'NONE':actionText=translate('LBL_PMSE_CONTEXT_MENU_SETTINGS');disabled=true;actionCSS='adam-menu-icon-configure';break;case'ASSIGN_USER':combo_users=new SearchableCombobox({label:translate('LBL_PA_FORM_LABEL_ASSIGN_TO_USER'),name:'act_assign_user',submit:true,searchURL:'pmse_Project/CrmData/users?filter={TERM}',required:true});updateRecordOwner=new CheckboxField({name:'act_update_record_owner',label:translate('LBL_PA_FORM_LABEL_UPDATE_RECORD_OWNER'),required:false,value:false,options:{labelAlign:'right',marginLeft:80}});proxy=new SugarProxy({url:'pmse_Project/ActivityDefinition/'+this.id,uid:this.id,callback:null});items=[combo_users,updateRecordOwner];wWidth=550;wHeight=160;labelWidth='40%';actionText=translate('LBL_PMSE_CONTEXT_MENU_SETTINGS');actionCSS='adam-menu-icon-configure';callback={'loaded':function(data){var users,nValue=false,usersProxy=new SugarProxy(),aUsers=[{'text':'Select...','value':''}];root.canvas.emptyCurrentSelection();usersProxy.url='pmse_Project/CrmData/users';if(data&&data.act_assign_user){usersProxy.getData(null,{success:function(users){var theMatch,i;users=users.result||[];for(i=0;i<users.length;i+=1){if(users[i].value===data.act_assign_user){theMatch={text:users[i].text,value:users[i].value};break;}}
if(!theMatch){theMatch={text:data.act_assign_user,value:data.act_assign_user};}
...
updateRecordOwner.setValue(nValue);$(updateRecordOwner.html).children('input').prop('checked',nValue);}};windowTitle=translate('LBL_PMSE_FORM_TITLE_ASSIGN_USER')+': '+this.getName();break;case'ASSIGN_TEAM':combo_teams=new ComboboxField({jtype:'combobox',label:translate('LBL_PA_FORM_LABEL_ASSIGN_TO_TEAM'),name:'act_assign_team',submit:true,proxy:new SugarProxy({url:'pmse_Project/CrmData/teams/public',uid:'public',callback:null})});updateRecordOwner=new CheckboxField({name:'act_update_record_owner',label:translate('LBL_PA_FORM_LABEL_UPDATE_RECORD_OWNER'),required:false,value:false,options:{labelAlign:'right',marginLeft:200}});combo_method=new ComboboxField({jtype:'combobox',name:'act_assignment_method',label:translate('LBL_PMSE_FORM_LABEL_ASSIGNMENT_METHOD'),options:[{text:translate('LBL_PMSE_FORM_OPTION_ROUND_ROBIN'),value:'balanced'},{text:translate('LBL_PMSE_FORM_OPTION_SELF_SERVICE'),value:'selfservice'}],initialValue:'balanced',editable:false,readOnly:true});hidden_method=new HiddenField({name:'act_assignment_method',initialValue:'balanced'});proxy=new SugarProxy({url:'pmse_Project/ActivityDefinition/'+this.id,uid:this.id,callback:null});items=[combo_teams,updateRecordOwner,hidden_method];wWidth=550;wHeight=160;labelWidth='40%';actionText=translate('LBL_PMSE_CONTEXT_MENU_SETTINGS');actionCSS='adam-menu-icon-configure';callback={'loaded':function(data){var teams;root.canvas.emptyCurrentSelection();teams=combo_teams.proxy.getData(null,{success:function(teams){combo_teams.setOptions(teams.result);if(data){combo_teams.setValue(data.act_assign_team||teams.result[0].value);nValue=false;if(data.act_update_record_owner&&data.act_update_record_owner==1){nValue=true;}
updateRecordOwner.setValue(nValue);$(updateRecordOwner.html).children('input').prop('checked',nValue);}
App.alert.dismiss('upload');w.html.style.display='inline';}});}};windowTitle=translate('LBL_PMSE_FORM_TITLE_ASSIGN_TEAM')+': '+this.getName();break;case'CHANGE_FIELD':labelWidth='20%';navigableData={'edit':true};changeFieldsFn=function(){App.alert.show('upload',{level:'process',title:'LBL_LOADING',autoclose:false});updater_field.proxy.url='pmse_Project/CrmData/relatedfields/'+combo_modules.value;data=updater_field.proxy.getData({call_type:'CF',base_module:PROJECT_MODULE},{success:function(data){App.alert.dismiss('upload');if(data){updater_field.setOptions(data.result,true);}}});};combo_modules=new ComboboxField({label:translate('LBL_PMSE_FORM_LABEL_MODULE'),name:'act_field_module',submit:true,change:changeFieldsFn,proxy:new SugarProxy({url:'pmse_Project/CrmData/related/'+PROJECT_MODULE,uid:PROJECT_MODULE,callback:null})});updater_field=new UpdaterField({label:translate('LBL_PMSE_FORM_LABEL_FIELDS'),name:'act_fields',submit:true,decimalSeparator:SUGAR.App.config.defaultDecimalSeparator,numberGroupingSeparator:SUGAR.App.config.defaultNumberGroupingSeparator,proxy:new SugarProxy({url:'pmse_Project/CrmData/fields/'+PROJECT_MODULE,uid:null,callback:null}),fieldWidth:470,fieldHeight:260,hasCheckbox:true});wWidth=670;wHeight=400;actionText=translate('LBL_PMSE_CONTEXT_MENU_SETTINGS');actionCSS='adam-menu-icon-configure';items=[combo_modules,updater_field];proxy=new SugarProxy({url:'pmse_Project/ActivityDefinition/'+this.id,uid:this.id,callback:null});windowTitle=translate('LBL_PMSE_FORM_TITLE_CHANGE_FIELDS')+': '+this.getName();callback={'loaded':function(data){var modules,opt=[],listProxy;root.canvas.emptyCurrentSelection();combo_modules.proxy.getData({cardinality:'one'},{success:function(modules){if(modules&&modules.success){combo_modules.setOptions(modules.result);initialModule=data.act_field_module||modules.result[0].value;project.addMetadata('projectModuleFields',{dataURL:'pmse_Project/CrmData/fields/'+PROJECT_MODULE,dataRoot:'result',success:function(data){updater_field.setVariables(data);}});updater_field.proxy.uid=PROJECT_MODULE;updater_field.proxy.url='pmse_Project/CrmData/relatedfields/'+initialModule;updater_field.proxy.getData({call_type:'CF',base_module:PROJECT_MODULE},{success:function(fields){if(fields){updater_field.setOptions(fields.result,true);updater_field.setValue(data.act_fields||null);App.alert.dismiss('upload');w.html.style.display='inline';}}});}}});}};break;case'ADD_RELATED_RECORD':labelWidth='20%';navigableData={'edit':true};changeFieldsFn=function(){App.alert.show('upload',{level:'process',title:'LBL_LOADING',autoclose:false});updater_field.proxy.uid=combo_modules.value;updater_field.proxy.url='pmse_Project/CrmData/addRelatedRecord/'+combo_modules.value;updater_field.proxy.getData({base_module:PROJECT_MODULE},{success:function(data){App.alert.dismiss('upload');if(data){updater_field.setOptions(data.result,true);}}});};combo_modules=new ComboboxField({jtype:'combobox',label:translate('LBL_PMSE_FORM_LABEL_RELATED_MODULE'),name:'act_field_module',submit:true,change:changeFieldsFn,proxy:new SugarProxy({url:'pmse_Project/CrmData/related/'+PROJECT_MODULE,uid:PROJECT_MODULE,callback:null})});updater_field=new UpdaterField({label:translate('LBL_PMSE_FORM_LABEL_FIELDS'),name:'act_fields',submit:true,decimalSeparator:SUGAR.App.config.defaultDecimalSeparator,numberGroupingSeparator:SUGAR.App.config.defaultNumberGroupingSeparator,proxy:new SugarProxy({url:'pmse_Project/CrmData/addRelatedRecord/'+PROJECT_MODULE,uid:null,callback:null}),fieldWidth:470,fieldHeight:260});wWidth=680;wHeight=420;actionText=translate('LBL_PMSE_CONTEXT_MENU_SETTINGS');actionCSS='adam-menu-icon-configure';items=[combo_modules,updater_field];proxy=new SugarProxy({url:'pmse_Project/ActivityDefinition/'+this.id,uid:this.id,callback:null});windowTitle=translate('LBL_PMSE_FORM_TITLE_ADD_RELATED_RECORD')+': '+this.getName();callback={'loaded':function(data){var modules,opt=[],listProxy;root.canvas.emptyCurrentSelection();combo_modules.proxy.getData({cardinality:'one-to-many'},{success:function(modules){if(modules&&modules.success&&modules.result&&modules.result.length>1){modules.result=modules.result.splice(1);combo_modules.setOptions(modules.result);initialModule=data.act_field_module||modules.result[0].value;updater_field.proxy.uid=PROJECT_MODULE;updater_field.proxy.url='pmse_Project/CrmData/addRelatedRecord/'+initialModule;project.addMetadata('projectModuleFieldsRelated',{dataURL:'pmse_Project/CrmData/fields/'+PROJECT_MODULE
+"?base_module="+PROJECT_MODULE,dataRoot:'result',success:function(data){updater_field.setVariables(data);}});updater_field.proxy.getData({base_module:PROJECT_MODULE},{success:function(fields){updater_field.setOptions(fields.result);updater_field.setValue(data.act_fields||null);App.alert.dismiss('upload');w.html.style.display='inline';}});}
else{App.alert.dismiss('upload');w.hide();App.alert.show('upload',{level:'warning',messages:SUGAR.App.lang.get('LBL_PMSE_CANNOT_CONFIGURE_ADD_RELATED_RECORD','pmse_Project'),autoClose:false});}}});}};break;case'BUSINESS_RULE':combo_business=new ComboboxField({label:translate('LBL_PMSE_LABEL_RULE'),name:'act_fields',submit:true,proxy:new SugarProxy({url:'pmse_Project/CrmData/rulesets/'+adamUID,uid:adamUID,callback:null})});hidden_module=new HiddenField({name:'act_field_module',initialValue:PROJECT_MODULE});wWidth=500;wHeight=140;actionText=translate('LBL_PMSE_CONTEXT_MENU_SETTINGS');actionCSS='adam-menu-icon-configure';items=[combo_business,hidden_module];proxy=new SugarProxy({url:'pmse_Project/ActivityDefinition/'+this.id,uid:this.id,callback:null});windowTitle=translate('LBL_PMSE_FORM_TITLE_BUSINESS_RULE')+': '+this.getName();callback={'loaded':function(data){var aRules=[{'text':translate('LBL_PMSE_FORM_OPTION_SELECT'),'value':'','selected':true}];root.canvas.emptyCurrentSelection();combo_business.proxy.getData(null,{success:function(rules){if(rules&&rules.success){aRules=aRules.concat(rules.result);combo_business.setOptions(aRules);if(data){combo_business.setValue(data.act_fields||'');}}
App.alert.dismiss('upload');w.html.style.display='inline';}});}};break;}
f=new Form({proxy:proxy,items:items,closeContainerOnSubmit:true,footerAlign:'right',buttons:[{jtype:'normal',caption:translate('LBL_PMSE_BUTTON_SAVE'),handler:function(){if(fields_updater&&fields_updater.multiplePanel){fields_updater.multiplePanel.close();}
f.submit();},cssClasses:['btn btn-primary']},{jtype:'normal',caption:translate('LBL_PMSE_BUTTON_CANCEL'),handler:function(){if(fields_updater&&fields_updater.multiplePanel){fields_updater.multiplePanel.close();}
if(f.isDirty()){cancelInformation.setButtons([{jtype:'normal',caption:translate('LBL_PMSE_BUTTON_YES'),handler:function(){cancelInformation.hide();w.close();}},{jtype:'normal',caption:translate('LBL_PMSE_BUTTON_NO'),handler:function(){cancelInformation.hide();}}]);cancelInformation.show();}else{w.close();}},cssClasses:['btn btn-invisible btn-link']}],labelWidth:labelWidth,callback:callback,language:PMSE_DESIGNER_FORM_TRANSLATIONS});w=new Window({width:wWidth,height:wHeight,title:windowTitle,modal:true});w.addPanel(f);action=new Action({text:actionText,cssStyle:actionCSS,handler:function(){root.canvas.project.save();w.show();w.html.style.display='none';App.alert.show('upload',{level:'process',title:'LBL_LOADING',autoclose:false});},disabled:disabled});return action;};
...
break;case'select':element=$('<select>').css({width:"100%"});for(i=0;i<this.options.length;i++)
{name=this.options[i].label;element.append($('<option label="'+name+'" value="'+this.options[i].value+'" '+((this.options[i].value===this.getValue())?'selected="selected"':'')+' '+(this.options[i].disabled?'disabled':'')+' >'+name+'</option>'));}
...
element.on('focusout',function(){that.save();}).on("click",function(e){e.stopPropagation();}).on("keydown",function(e){e.stopPropagation();if(e.keyCode===13)
{that.save();}else if(e.keyCode===27){discardChanges.call(that);}});try{$(html).find('td:last').empty();}catch(e){};$(html).find('td:last').append(element);element.focus();if(typeof this.onEditMode==='function'){this.onEditMode.call(this);}

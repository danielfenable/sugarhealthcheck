...
for(i=this.stackCommandConnect.length-1;i>=0;i-=1){this.stackCommandConnect[i].buildConnection();}
...
return false;};};NoDropBehavior=function(selectors){DropBehavior.call(this,selectors);};NoDropBehavior.prototype=new DropBehavior();NoDropBehavior.prototype.type="NoDropBehavior";NoDropBehavior.prototype.attachDropBehavior=function(shape){var $shape=$(shape.getHTML());DropBehavior.prototype.attachDropBehavior.call(this,shape);$(shape).droppable('accept',"");};ContainerDropBehavior=function(selectors){DropBehavior.call(this,selectors);};ContainerDropBehavior.prototype=new DropBehavior();ContainerDropBehavior.prototype.type="ContainerDropBehavior";ContainerDropBehavior.prototype.defaultSelector=".custom_shape";ContainerDropBehavior.prototype.onDrop=function(shape){return function(e,ui){var customShape=null,canvas=shape.getCanvas(),selection,sibling,i,command,coordinates,id,shapesAdded=[],containerBehavior=shape.containerBehavior;if(canvas.readOnly){return false;}
...
if(SEGMENT===3){var etx=0;var ety=0;if(newPointA.y<newPointB.y)
{etx=this.positive(newPointA.y-newPointB.y);}
if(newPointA.x<newPointB.x)
{ety=this.positive(newPointA.x-newPointB.x);}
if(toDir===0){if(toDir===0&&fromDir===0){if(newPointA.y<newPointB.y)
{findPoint=new Point(newPointA.x,newPointA.y-this.mindist);}
else
{etx=this.positive(newPointA.y-newPointB.y);findPoint=new Point(newPointA.x,newPointA.y-(this.mindist+etx));}}else{var newAux=0;newAux=(newPointA.y+newPointB.y)/2;findPoint=new Point(newPointA.x,newAux);}}
...
if(toDir===3){if(toDir===3&&fromDir===3){if(newPointA.x<newPointB.x)
{findPoint=new Point(newPointA.x-this.mindist,newPointA.y);}
else
{ety=this.positive(newPointA.x-newPointB.x);findPoint=new Point(newPointA.x-(this.mindist+ety),newPointA.y);}}else{var newAux=0;newAux=(newPointA.x+newPointB.x)/2;findPoint=new Point(newAux,newPointA.y);}}
newPointA=findPoint;}else if(SEGMENT==2){if(thor==false){if(fromDir===0||fromDir===2)
{findPoint=new Point(newPointB.x,newPointA.y);}else if(fromDir===1||fromDir===3)
{findPoint=new Point(newPointA.x,newPointB.y);}}else{if(fromDir===0||fromDir===2)
{if(newPointA.x<newPointB.x&&newPointA.y>newPointB.y){if(fromDir===2){findPoint=new Point(newPointB.x,newPointA.y);}else{findPoint=new Point(newPointA.x,newPointB.y);}}else{findPoint=new Point(newPointB.x,newPointA.y);}}else if(fromDir===1||fromDir===3)
{if(newPointA.x-newPointB.x>newPointA.y-newPointB.y){findPoint=new Point(newPointA.x,newPointB.y);}else{findPoint=new Point(newPointA.x,newPointB.y);}}}}
...
customShape.wasDragged=false;e.stopPropagation();if(listPanelError!==undefined){erros=customShape.BPMNError.asArray();if(erros.length){id=customShape.getID();item=listPanelError.items.filter(function(item){if(item.getErrorId()===id){return item}});if(item.length){item=item[0];item.select();$("#div-bpmn-error")[0].scrollTop=item.html.offsetTop;}}else{if(listPanelError.selectedItem){listPanelError.selectedItem.deselect();}}}};};CustomShape.prototype.parseHook=function(){};CustomShape.prototype.getPorts=function(){return this.ports;};CustomShape.prototype.getLayers=function(){return this.layers;};CustomShape.prototype.getLabels=function(){return this.labels;};CustomShape.prototype.applyZoom=function(){var i,label;Shape.prototype.applyZoom.call(this);for(i=0;i<this.layers.getSize();i+=1){this.layers.get(i).applyZoom();}
...
this.html="";};CustomLine.prototype.createDiv=function(x,y,w,h,transform){var orientation=(w===1?1:(h===1?0:-1)),offset;if(transform){offset={left:x,top:y};}else{offset={left:x-(orientation===1?1:0),top:y-(orientation===0?1:0)};}
this.html=$('<div>').addClass('line').css({'animation':'none','position':'absolute'}).width(w).height(h).offset(offset);if(transform){$(this.html).css({'-webkit-transform':transform,'-moz-transform':transform,'-ms-transform':transform,'transform':transform});}else{$(this.html).addClass((orientation===1?'line-vertical':(orientation===0?'line-horizontal':'')));}};CustomLine.prototype.remove=function(){if(this.html){$(this.html).remove();}};CustomLine.prototype.clear=function(){$(this.html).empty();if(this.canvas)this.canvas.innerHTML="";};function onCanvasKeyDown(e){if(activeCanvas){switch(e.which){case 16:isShift=true;break;case 17:isCtrl=true;break;case 116:e.preventDefault();window.location.reload(true);break;case 37:if(!activeCanvas.currentLabel){e.preventDefault();activeCanvas.moveElements(activeCanvas,'LEFT');}

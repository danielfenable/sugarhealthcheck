			...
			<div>				
				<table width="100%" border="0" cellspacing="0" cellpadding="0" >
					...
					<tr>
						...
						<td scope="row">
						<span sugar='slot10'>{$MOD.LBL_IS_EXTERNAL_ARTICLE}</span sugar='slot'>
						&nbsp;</td>
						<td  width="35%">
							<span sugar="slot10b">
								<input name="is_external_flag" id="is_external_flag" onclick="externalChecked();" class="checkbox" type="checkbox" {$is_external_checked}>
								<input name="is_external_article" id="is_external_article" type="hidden" value="{$is_external_article}">
							&nbsp;</span>
...
{literal}				
	...
	function externalChecked(){
		if(document.getElementById('is_external_flag').checked){
			document.getElementById('is_external_article').value = 1;
		}else{
			document.getElementById('is_external_article').value = 0;	
		}
	}

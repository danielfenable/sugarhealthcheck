...
result.push(value);}
return result;};CAL.openActivityCreateDrawer=function(module,prefillAttributes){var prefill=app.data.createBean(module);prefill.set(prefillAttributes);app.drawer.open({layout:'create',context:{create:true,model:prefill,module:module,forceNew:true}},_.bind(function(context,model){app.alert.dismiss('create-option-alert');if(model){document.location.reload();}},this));};CAL.loadActivityCreateDrawer=function(module,prefillAttributes){var prefill=app.data.createBean(module);prefill.set(prefillAttributes);app.drawer.load({layout:'create',context:{create:true,model:prefill,module:module,forceNew:true}});};CAL.unformatDateTime=function(dateTime){var dateFormat=app.user.getPreference('datepref')+' '+app.user.getPreference('timepref'),jsDateFormat=app.date.convertFormat(dateFormat);return app.date(dateTime,jsDateFormat,true).format();};YAHOO.util.DDCAL=function(id,sGroup,config){this.cont=config.cont;YAHOO.util.DDCAL.superclass.constructor.apply(this,arguments);}

<?php
if(!defined('sugarEntry') || !sugarEntry) die('Not A Valid Entry Point');


/*
 * Your installation or use of this SugarCRM file is subject to the applicable
 * terms available at
 * http://support.sugarcrm.com/06_Customer_Center/10_Master_Subscription_Agreements/.
 * If you do not agree to all of the applicable terms or do not have the
 * authority to bind the entity as an authorized representative, then do not
 * install or use this SugarCRM file.
 *
 * Copyright (C) SugarCRM Inc. All rights reserved.
 */


$mod_strings = array (
  'LBL_ACTION_ERROR' => 'Esta acción no puede ser ejecutada. Edite la acción para que todos los campos y valores sean válidos',
  'LBL_ACTION_ERRORS' => 'Aviso: Una o más de las siguientes acciones contiene errores.',
  'LBL_ALERT_ERROR' => 'Esta alerta no puede ser ejecutada. Edite la alerta para que todas sus opciones de configuración sean válidas.',
  'LBL_ALERT_ERRORS' => 'Aviso: Una o más de las siguientes alertas contiene errores.',
  'LBL_ALERT_SUBJECT' => 'ALERTA DE WORKFLOW',
  'LBL_ALERT_TEMPLATES' => 'Plantillas de Alertas',
  'LBL_ANY_FIELD' => 'cualquier campo',
  'LBL_AS' => 'como',
  'LBL_BASE_MODULE' => 'Módulo Objetivo:',
  'LBL_BODY' => 'Cuerpo:',
  'LBL_CREATE_ALERT_TEMPLATE' => 'Crear una plantilla de alerta:',
  'LBL_DESCRIPTION' => 'Descripción:',
  'LBL_DOWN' => 'Abajo',
  'LBL_EDITLAYOUT' => 'Editar diseño',
  'LBL_EDIT_ALT_TEXT' => 'Texto Alt',
  'LBL_EMAILTEMPLATES_TYPE' => 'Tipo',
  'LBL_EMAILTEMPLATES_TYPE_LIST_WORKFLOW' => 
  array (
    'workflow' => 'Workflow',
  ),
  'LBL_FIRE_ORDER' => 'Orden de Proceso:',
  'LBL_FROM_ADDRESS' => 'Dirección del Remitente:',
  'LBL_FROM_NAME' => 'Nombre del Remitente:',
  'LBL_HIDE' => 'Ocultar',
  'LBL_INSERT' => 'Insertar',
  'LBL_INVITEES' => 'Asistentes',
  'LBL_INVITEE_NOTICE' => 'Atención, debe de seleccionar al menos un asistente para crear esto.',
  'LBL_INVITE_LINK' => 'Enlace de Invitación a Reunión/Llamada',
  'LBL_LACK_OF_NOTIFICATIONS_ON' => 'Advertencia: Para enviar alertas, proporcione la información sobre el Servidor SMTP en Admin > Configuración de Email.',
  'LBL_LACK_OF_TRIGGER_ALERT' => 'Advertencia: Debe crear un disparador para que funcione este objeto de workflow',
  'LBL_LINK_RECORD' => 'Vincular con Registro',
  'LBL_LIST_BASE_MODULE' => 'Módulo Objetivo:',
  'LBL_LIST_DN' => 'ab',
  'LBL_LIST_FORM_TITLE' => 'Lista de Workflows',
  'LBL_LIST_NAME' => 'Nombre',
  'LBL_LIST_ORDER' => 'Órden de Proceso:',
  'LBL_LIST_STATUS' => 'Estado',
  'LBL_LIST_TYPE' => 'Ejecución al:',
  'LBL_LIST_UP' => 'ar',
  'LBL_MODULE_ID' => 'Workflow',
  'LBL_MODULE_NAME' => 'Definiciones de Workflow',
  'LBL_MODULE_NAME_SINGULAR' => 'Definición de Workflow',
  'LBL_MODULE_TITLE' => 'Workflow: Inicio',
  'LBL_NAME' => 'Nombre:',
  'LBL_NEW_FORM_TITLE' => 'Crear Definición de Workflow',
  'LBL_PLEASE_SELECT' => 'Por favor, Seleccione',
  'LBL_PROCESS_LIST' => 'Secuencia de Workflow',
  'LBL_PROCESS_SELECT' => 'Por favor, seleccione un módulo:',
  'LBL_RECIPIENTS' => 'Destinatarios',
  'LBL_RECORD_TYPE' => 'Aplica a:',
  'LBL_RELATED_MODULE' => 'Módulo Relacionado:',
  'LBL_SEARCH_FORM_TITLE' => 'Búsqueda de Workflows',
  'LBL_SELECT_FILTER' => 'Debe seleccionar un campo de filtrado para el módulo relacionado.',
  'LBL_SELECT_MODULE' => 'Por favor, seleccione un módulo relacionado.',
  'LBL_SELECT_OPTION' => 'Por favor, seleccione una opción.',
  'LBL_SELECT_VALUE' => 'Debe seleccionar un valor.',
  'LBL_SET' => 'Establecer',
  'LBL_SHOW' => 'Mostrar',
  'LBL_SPECIFIC_FIELD' => 'campo especifico',
  'LBL_STATUS' => 'Estado:',
  'LBL_SUBJECT' => 'Asunto:',
  'LBL_TRIGGER_ERROR' => 'Aviso: Este disparador contiene valores no válidos y no se disparará.',
  'LBL_TRIGGER_ERRORS' => 'Aviso: Uno o más de los siguientes disparadores contiene errores.',
  'LBL_TYPE' => 'Ejecución al:',
  'LBL_UP' => 'Arriba',
  'LBL__S' => 's',
  'LNK_ALERT_TEMPLATES' => 'Plantillas de Emails de Alerta',
  'LNK_NEW_WORKFLOW' => 'Crear Definición de Workflow',
  'LNK_PROCESS_VIEW' => 'Secuencia de Workflow',
  'LNK_WORKFLOW' => 'Listar Definiciones de Workflow',
  'NTC_REMOVE_ALERT' => '¿Está seguro de que quiere quitar este workflow?',
  'WARN_WORKFLOW_DEPRECATED_ERROR' => '<b>Advertencia:</b> El módulo del flujo de trabajo quedará pronto obsoleto y se eliminará. Transfiera los flujos de trabajo al nuevo <a href="index.php?module=pmse_Project">Autor de procesos</a>.<span style="width: 1em; display: inline-block;">&nbsp;</span><a target="_blank" href="http://support.sugarcrm.com/02_Documentation/01_Sugar_Editions/01_Sugar_Ultimate/Sugar_Ultimate_7.6/Administration_Guide/135_Process_Author/">Obtenga más información acerca del Autor de procesos</a>.',
);


...
{var loader=new YAHOO.util.YUILoader({require:["element","sugarwidgets"],loadOptional:true,skin:{base:'blank',defaultSkin:''},onSuccess:testOutboundSettings,allowRollup:true,base:"include/javascript/yui/build/"});loader.addModule({name:"sugarwidgets",type:"js",fullpath:"include/javascript/sugarwidgets/SugarYUIWidgets.js?v="+(SUGAR.VERSION_MARK||""),varName:"YAHOO.SUGAR",requires:["datatable","dragdrop","treeview","tabview"]});loader.insert();}
...
EmailMan.testOutboundDialog.hide();overlay(SUGAR.language.get('app_strings',"LBL_EMAIL_PERFORMING_TASK"),SUGAR.language.get('app_strings',"LBL_EMAIL_ONE_MOMENT"),'alert');var callbackOutboundTest={success:function(o){hideOverlay();overlay(SUGAR.language.get('app_strings',"LBL_EMAIL_TEST_OUTBOUND_SETTINGS"),SUGAR.language.get('app_strings',"LBL_EMAIL_TEST_NOTIFICATION_SENT"),'alert');}};var smtpUser=document.getElementById('mail_smtpuser');var smtpPass=document.getElementById('mail_smtppass');var smtpServer=document.getElementById('mail_smtpserver').value;var postDataString;if(smtpUser&&smtpPass){smtpUser=trim(smtpUser.value);smtpPass=trim(smtpPass.value);postDataString='mail_sendtype=SMTP&mail_smtpserver='+smtpServer+"&mail_smtpport="+mail_smtpport+"&mail_smtpssl="+mail_smtpssl+"&mail_smtpauth_req=true&mail_smtpuser="+smtpUser+"&mail_smtppass="+encodeURIComponent(smtpPass)+"&outboundtest_from_address="+fromAddress+"&outboundtest_to_address="+toAddress;}else{postDataString='mail_sendtype=SMTP&mail_smtpserver='+smtpServer+"&mail_smtpport="+mail_smtpport+"&mail_smtpssl="+mail_smtpssl+"&outboundtest_from_address="+fromAddress+"&outboundtest_to_address="+toAddress;}
...
{var isError=!check_form("EditView");if(document.getElementById("required_password").value=='1'&&document.getElementById("new_password").value==""){add_error_style('EditView',form.new_password.name,SUGAR.language.get('app_strings','ERR_MISSING_REQUIRED_FIELDS')+SUGAR.language.get('Users','LBL_NEW_PASSWORD'));isError=true;}
...
if(window.parent.SUGAR&&window.parent.SUGAR.App&&(!document.EditView.isDuplicate||!document.EditView.isDuplicate.value)){var field=document.getElementById('picture');if(field){var filename=field.value;if(!filename||filename&&!filename.length){window.parent.SUGAR.App.events.trigger("bwc:avatar:removed");}}}
return true;}
function submit_form(form){var action;action=form.action.value;form.action.value='validate';$("#SAVE_HEADER").attr('disabled','disabled');$("#SAVE_FOOTER").attr('disabled','disabled');$.ajax({type:'POST',data:$(form).serialize(),success:function(data){if(!data.status){$('#ajax_error_string').text(data.error_string);}else{$('#ajax_error_string').hide();form.action.value=action;form.submit();}},complete:function(){setTimeout(function(){$("#SAVE_HEADER").removeAttr('disabled');$("#SAVE_FOOTER").removeAttr('disabled');},100);}});}
...
function onUserEditView(){YAHOO.util.Event.onContentReady('user_theme_picker',function(){document.getElementById('user_theme_picker').onchange=function(){document.getElementById('themePreview').src="index.php?entryPoint=getImage&themeName="+document.getElementById('user_theme_picker').value+"&imageName=themePreview.png";if(themeGroupList&&typeof themeGroupList[document.getElementById('user_theme_picker').value]!='undefined'&&themeGroupList[document.getElementById('user_theme_picker').value]){document.getElementById('use_group_tabs_row').style.display='';}else{document.getElementById('use_group_tabs_row').style.display='none';}}});setSymbolValue(document.getElementById('currency_select').options[document.getElementById('currency_select').selectedIndex].value);setSigDigits();user_status_display(document.getElementById('UserType'));setSugarClientEmailOption();}
function setSugarClientEmailOption(){var emailType=document.getElementById('email_link_type'),isSugarClientDisabled=emailType.getAttribute('data-sugarclientdisabled');if(isSugarClientDisabled&&isSugarClientDisabled==='true'){var option=emailType.getElementsByTagName('option');for(var i=0;i<option.length;i++){if(option[i].value.toLowerCase()==='sugar'){option[i].disabled=true;}}}}